...
<script>
    ...
    var table = $('#volunteersTable').dataTable({
        "bFilter": false,
        "ajax": $('meta[name=apiUrl]').attr('content') + '/missions',
        "columns": [
            {data: "id"},
            {
                //concat first name with last name
                data: null, render: function (data, type, row) {
                return '<a href="' + $("body").attr('data-url') + '/missions/one/' + data.id + '">' + data.name + ' ' + data.last_name + '</a>';
            }
            },
            {
        ...
        dom: 'T<"clear">lfrtip',
        "tableTools": {
            "sSwfPath": $("body").attr('data-url') + "/plugins/data-tables/extras/tabletools/swf/copy_csv_xls_pdf.swf",
            "aButtons": [
                {
                    "sExtends": "copy",
